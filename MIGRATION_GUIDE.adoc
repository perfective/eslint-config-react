...
+
** Rename `eslint.config.js` into `eslint.config.js`.
+
...
+
For example, change `rxjs/ban-observables` to `rxjs-x/ban-observables`.
+
* *Update imports and rules configuration*:
+
** All configs are now functions.
** Rules functions from `@perfective/eslint-config-react/rules` are moved into the corresponding config packages.
** `typescriptEslintNamingConvention` is moved from `@perfective/eslint-config-react/rules`
to `@perfective/eslint-config-react/typescript-eslint`.
+
* *Import the following functions directly from `@perfective/eslint-config`* (if used):
+
...
export default perfectiveEslintReactConfig([
    // ...Optional plugins configs...
    {
        rules: {
            ...
            // If eslint-plugin-cypress is used
            'cypress/no-xpath': 'off',

            // If eslint-plugin-jest is used
...
* Disable new rules in the `eslint.config.js`.
+
[source,js]
----
module.exports = {
